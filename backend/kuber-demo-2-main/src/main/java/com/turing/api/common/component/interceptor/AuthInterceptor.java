...
//            log.info("1-인터센트 토큰 로그 Bearer 포함 : {}", token);
//            if(token.equals("undefined")){
//                response.sendError(HttpServletResponse.SC_BAD_REQUEST);
//                return false;
//            }
//            Long id = jwtProvider.getPayload(token).get("user_id", Long.class);
...
//        log.info("4- 인터셉터 최종 여부 {} ", true);
      return Stream.of(request)
              ...
              .peek(token -> log.info("1-인터센트 토큰 로그 Bearer 포함 : {}", token))
               .map(user-> jwtProvider.getPayload(user).get("userId", Long.class))
